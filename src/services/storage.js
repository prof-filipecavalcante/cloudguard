...
export function loadStudentProgress(studentId) {
  ...
  const attempts = Number.parseInt(localStorage.getItem(key('attempts', studentId)) || '0', 10);
  ...
  return { completed: Array.isArray(completed) ? completed.filter(Number.isInteger) : [], attempts: Number.isFinite(attempts) ? Math.min(Math.max(attempts, 0), 3) : 0, deadline };
}
...
export function saveAttempts(studentId, attempts) { localStorage.setItem(key('attempts', studentId), String(attempts)); }
